...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            p = Path(p)  # to Path
            print(str(save_dir / p.name),str(save_dir / 'labels' / p.stem))
                    ...
                    if conf>0.3:
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            if names[int(cls)]=='people':
                                label=f'hat {conf:.2f}'
                                # label = f'hat'
                            elif names[int(cls)]=='smoke_up':
                                label = f'person {conf:.2f}'
                                # label = f'person'
                            # label = f'{names[int(cls)]} {conf:.2f}'
                            # print(label)
                            # print(1111111111111,int(cls),colors[int(cls)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='best_SE_NON.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='E:/Server download/images', help='source')  # file/folder, 0 for webcam
